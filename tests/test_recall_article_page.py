...
def test_facebook_share(recall_article_page):
    ...
    assert TestData.FACEBOOK_SHARER_URL + TestData.RECALL_ARTICLE_URL == recall_article_page.get_facebook_share_href()
...
def test_twitter_share(recall_article_page):
    ...
    assert f"{TestData.TWITTER_SHARER_URL}text={title}&via=ConsumerAffairs&url={TestData.RECALL_ARTICLE_URL}" == href


def test_email_share(recall_article_page):
    """ Asserts email share button's href is right."""
    assert TestData.EMAIL_SHARER_URL + TestData.RECALL_ARTICLE_URL == recall_article_page.get_email_share_href()
...
def test_find_my_match(recall_article_page):
    ...
    assert recall_article_page.driver.current_url == TestData.FIND_MY_MATCH_URL + TestData.ZIP_CODE
